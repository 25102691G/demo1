...
class BaiduDocParserClient:
    ...
    def parse_pdf(self, pdf_path: str | Path) -> list[TextPage]:
        ...
        if artifacts.parse_result_text:
            parse_result = _loads_json_object(artifacts.parse_result_text)
            pages = _pages_from_parse_result(parse_result)
    ...
    def _submit_task(self, submit_url: str, access_token: str, path: Path) -> str:
        ...
        # 文档切分参数，按照语义分块（效果不好，暂时不启用）
        # return_doc_chunks = json.dumps({"switch": True, "chunk_size": -1})
        # payload["return_doc_chunks"] = return_doc_chunks
    ...
    def _download_artifacts(
        ...
        if isinstance(parse_result_url, str) and parse_result_url:
            ...
            parse_result = _loads_json_object(parse_result_text)
            pages = _pages_from_parse_result(parse_result)
            _debug(
        ...
        if isinstance(markdown_url, str) and markdown_url:
            markdown = self._get_text(markdown_url)
    ...
    def _get_access_token(self) -> str:
        ...
        result = self._post_json(token_url, payload)
