...
推荐使用 `guideline_skill.cli` 从指南 PDF 抽取 `result.jsonl`。这是当前和后续 `scripts/build_skill_pack.py` 对接的主路径。
...
## 3. OCR parse_result.json 到 recommendation_card.jsonl
...
```powershell
python scripts/ocr_to_cards.py --input .\data\skills\肠结核的诊断与治疗\肠结核的诊断与治疗.parse_result.json --llm-workers 4
```

批量转换：

```powershell
python scripts/ocr_to_cards.py --input-dir .\data\skills --llm-workers 4
```

`--input` 和 `--input-dir` 必须且只能指定一个。未指定 `--output` 时，脚本默认写入输入 `*.parse_result.json` 所在目录的 `recommendation_card.jsonl`；`--input-dir` 模式下会为每个输入文件分别写入各自目录。显式指定 `--output` 时，会把所有输入合并写入该 JSONL 文件。

`--llm-workers` 用于并发清洗同一个文件内的 text layout，默认 `1`。例如 `--llm-workers 4` 会同时发起最多 4 个 LLM 请求。

批量运行时，脚本会在控制台打印当前处理的文件序号、剩余文件数、LLM 清洗进度和每个文件的输出结果。
...
- `--case-schema`：canonical case schema 路径，默认 `schema/canonical_case.schema.json`
- `--skill-schema`：skill pack schema 路径，默认 `schema/skill_pack.schema.json`
- `--output-schema`：workflow output schema 路径，默认 `schema/workflow_output.schema.json`
- `--debug`：输出 debug trace
